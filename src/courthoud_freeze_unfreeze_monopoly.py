#!/usr/bin/env python3
"""
courthoud_freeze_unfreeze_monopoly_onefile.py
=============================================
One-file Courthoud-style freeze/unfreeze experiment + plotting script.

This script combines:
1. Courthoud-style freeze/unfreeze simulation logic.
2. Total Revenue plotting with a 1/4 Monopoly benchmark line.
...
5. Unfreeze the three frozen platforms and let all four platforms learn again.
6. Plot Total Revenue with:
   - Platform 1 = blue
   - Platform 2 = orange
   - Platform 3 = green
   - Platform 4 = red
...
---------------------
- Pre-freeze evaluation: 70 episodes
- Freeze phase: 70 episodes
- Post-unfreeze phase: 70 episodes

Example
-------
python courthoud_freeze_unfreeze_monopoly_onefile.py \
  --data-dir data \
  --out-dir experiments/courthoud_freeze_unfreeze_monopoly \
  --episodes 100 \
  --baseline-eval-episodes 70 \
  --freeze-episodes 70 \
  --post-unfreeze-episodes 70 \
  --steps 120 \
  --deviator 0 \
  --monopoly-csv tests/logs/monopoly_rewards_history.csv \
  --monopoly-column Revenue \
  --monopoly-scale 0.25
"""
...
def plot_revenue(df: pd.DataFrame, out_dir: Path, deviator: int, ma_window: int, dpi: int, monopoly: Optional[np.ndarray], fleet_sizes: list[int]) -> None:
    ...
    for firm in range(N_FIRMS):
        ...
        role = "deviator / continuous learning" if is_dev else "frozen, then unfrozen"
    ...
    ax.set_title(f"Courthoud Defection Test Revenue per Taxi Comparison — Platform {deviator+1} Deviates", fontsize=18, weight="bold")
    ...
    add_phase_lines(ax, freeze, unfreeze)
    ax.set_title(f"Courthoud Defection Test Cumulative Revenue per Taxi — Platform {deviator+1} Deviates", fontsize=18, weight="bold")
    ax.set_xlabel("Episode", fontsize=14)
...
def save_summary(df: pd.DataFrame, out_dir: Path, deviator: int, fleet_sizes: list[int]) -> None:
    ...
    for firm in range(N_FIRMS):
        ...
        rows.append({
            ...
            "firm_id": firm,
            "role": "deviator" if firm == deviator else "frozen_then_unfrozen",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="One-file Courthoud freeze/unfreeze experiment with 1/4 monopoly benchmark plot")
    parser.add_argument("--data-dir", type=str, default="data")
    parser.add_argument("--out-dir", type=str, default="experiments/courthoud_freeze_unfreeze_monopoly")
